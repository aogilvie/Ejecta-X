...
Handle<Object> tephra_getNativeClass(JSContextRef ctx, JSObjectRef object, JSStringRef propertyNameJS, JSValueRef* exception) {
	...
	if (pClass) {
		...
		// Make persistent ?
		// Persistent<Value> value = Persistent<Value>::new(obj);

		obj->Set(String::NewFromUtf8(isolate, fullClassName->getCString()), tephra_constructorClass->GetFunction());
...
EJApp::EJApp() : currentRenderingContext(0), screenRenderingContext(0), touchDelegate(0), touches(0), openGLContext(NULL) {
	...
	jsGlobalIsolate = Isolate::New();
	Isolate::Scope isolate_scope(jsGlobalIsolate);

	// Create V8 Context
	jsGlobalContext = Context::New(jsGlobalIsolate);
	Persistent<Context> persistent_context(jsGlobalIsolate, jsGlobalContext);

	// Start context scope
	Context::Scope context_scope(jsGlobalContext);
...
void EJApp::loadScriptAtPath(NSString *path) {
	...
	// Evaluate a string containing the JavaScript source code.
	Local<String> source = String::NewFromUtf8(jsGlobalIsolate, path->getCString());
	Local<String> name(String::NewFromUtf8(jsGlobalIsolate, "loadScriptAtPath"));
	ExecuteString(jsGlobalIsolate, source, name, true, true);
...
bool ExecuteString(Isolate *isolate,
	...
	if (script.IsEmpty()) {
		...
		if (report_exceptions)
			EJException::ReportException(isolate, &try_catch);
		return false;
	} else {
		...
		if (result.IsEmpty()) {
			...
			if (print_result && !result->IsUndefined()) {
				...
				const char *cstr = EJException::ToCString(str);
				printf("%s\n", cstr);
			}
			return true;
		}
...
// TODO:
/*
Handle<Value> EJApp::loadModuleWithId(NSString *moduleId, JSValueRef module, JSValueRef exports) {
...
Handle<FunctionTemplate> EJApp::getJSClassForClass(EJBindingBase *classId) {
	// JSClassRef EJApp::getJSClassForClass(EJBindingBase* classId) {

	Handle<FunctionTemplate> jsClass = NULL;
	...
	if (jsClasses->count()) {
		...
		NSDICT_FOREACH(jsClasses, pElement) {
			...
			if ( key == classId->toString() ) {
				...
				jsClass = (Handle<FunctionTemplate>)((NSValue*)jsClasses->objectForKey(classId->toString()))->pointerValue();
...
// This method is now build into ExecuteString
/*
void EJApp::logException(JSValueRef valueAsexception, JSContextRef ctxp) {
    ...
    EJException::logExceptionError(exception);
